...
public class BotTextRepository {
    ...
    static private final String botManual = """
            ...
            Настройки:
            /setClearPeriod - позволяет настроить период между чистками сообщений. В сообщении с командой через пробел после нее укажите раз в сколько минут необходимо чистить сообщения.
            /getClearPeriod - позволяет ухнать текузий период между чистками соообщений.
            /autoSendTaskForAllEnable - делает рассылку задач отмечаемых пользователем автоматической. Автоматическая рассылка рассылает задачу во все привязанные каналы.
            /autoSendTaskForAllDisable - делает рассылку задач отмечаемых пользователем настраеваемой.
            /setCustomTaskMessage - устанавливает пользовательские настройки для сообщения отправляющего задачи. В сообщении с командой через пробел после нее укажите текст, который будет воспроизводить функционал команды /task.
            /canselCustomTaskMessage - отменяет пользовательские настройки для сообщения отправляющего задачи.
            """;
    @Getter
